...
namespace BuildingMonitor.Actors
{
  ...
  {
    string _floorId;
    Dictionary<string, IActorRef> _sensorIdToActorRefMap =
    ...
    protected override void OnReceive(object message)
    {
      switch (message)
      {
        case RequestRegisterTemperatureSensor m when m.FloorId == _floorId:
          if (_sensorIdToActorRefMap.TryGetValue(m.SensorId, out var sensorRef))
            sensorRef.Forward(m);
        ...
        case RequestTemperatureSensorIds m:
          ...
          break;
        case Terminated m:
          var terminatedId = _sensorIdToActorRefMap.First(x => x.Value == m.ActorRef).Key;
          _sensorIdToActorRefMap.Remove(terminatedId);
          break;
        default:
          Unhandled(message);
          break;
